# Smartsheet Closeout Database Schema
...
| instrument | TEXT | Default 'XCS' - parse from sheet_name instead |
| run | TEXT | Default '24' |
...
### Find experiments for a specific instrument
...
## Column Titles in Cells

Common column_title values:
- `Categories` - Section headers (Science, Administrative, Safety)
- `Topics` - Questions within each category
- `Comment` - Responses to topics
- `Action` - Action items
- `Action Comment` - Notes on actions
- `Action Owner` - Who is responsible
- `Action Status` - Status of action
- `Due Date` - Deadline
